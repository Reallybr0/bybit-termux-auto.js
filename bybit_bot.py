...
TEXTS = {
    "my": {
        ...
        "auto_claim_title": "🎁 AUTO CLAIM လုပ်ဆောင်နေပါသည်...",
...
class BybitBot:
    def __init__(self):
        ...
        # Browser ပုံစံတူစေရန် Anti-bot Cloudflare ရှောင်ကွင်းနိုင်မည့် Headers များဖြည့်စွက်ထားသည်
        self.headers = {
    ...
    def set_cookie(self, cookie_str):
        ...
        # အနာဂတ်မှာ Auto ပြန်ဖတ်နိုင်ရန် သိမ်းဆည်းပေးခြင်း
    ...
    def has_cookie(self): return self.cookie_string is not None
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("retCode") == 0:
                    ...
                    if items:
                        print(f"\n{"No.":<4} {"Reward Name":<30} {"Status":<12}")
                        print("─" * 50)
                        for idx, item in enumerate(items, 1):
                            print(f"{idx:<4} {item.get('name', 'N/A')[:28]:<30} {item.get('status', 'N/A'):<12}")
                    else:
                        print_info("မတွေ့ရှိပါ။")
                    return data
                else: 
                    print_error(f"{t('error_api')}{data.get('retMsg')} (Code: {data.get('retCode')})")
                    if data.get("retCode") == 10001:
                        print_warning("Cookie သက်တမ်းကုန်သွားခြင်း သို့မဟုတ် မှားယွင်းနေခြင်း ဖြစ်နိုင်ပါသည်။")
    ...
    def claim_all_rewards(self):
        ...
        for idx, item in enumerate(claimable, 1):
            ...
            else: print_error(f"{name} - {result.get('retMsg', t('claim_fail'))}")
            time.sleep(1.5)
    ...
    def run_spot_trade(self):
        ...
        while current_volume < SPOT_VOLUME_TARGET:
            try:
                ...
                print_error(f"Trade Blocked/Error: {e}")
                time.sleep(5)
...
def main():
    ...
    while True:
        ...
        # အခြေအနေပြ မီးပွိုင့်စနစ်ထည့်သွင်းခြင်း
        c_status = f"{ANSI.GREEN}{t('cookie_loaded')}{ANSI.RESET}" if bot.has_cookie() else f"{ANSI.RED}{t('cookie_not_loaded')}{ANSI.RESET}"
        a_status = f"{ANSI.GREEN}{t('api_loaded')}{ANSI.RESET}" if bot.has_api() else f"{ANSI.RED}{t('api_not_loaded')}{ANSI.RESET}"
        print(f"Status: [ {c_status} ] | [ {a_status} ]")
        print_divider()
        ...
        if choice == "1":
            ...
            print(TEXTS["my"]["enter_cookie_info"][0])
            c = input(f"{TEXTS['my']['enter_cookie_prompt']} ").strip()
            if c: bot.set_cookie(c)
        elif choice == "2":
            bot.load_cookie_from_file()
        elif choice == "3":
            k = input(t("enter_api_prompt_key")).strip()
            s = input(t("enter_api_prompt_secret")).strip()
            if k and s:
