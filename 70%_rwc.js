...
class BybitBot {
    ...
    openBybitApp() {
        ...
        try {
            ...
            // Uses Android monkey package command to forcefully start the app launcher activity
    ...
    // Option 8: Spot Trading Auto-Volume
    async runSpotTrade() {
        if (!this.hasApi()) return this.printError("API Key required.");
...
async function main() {
    ...
    while (true) {
        ...
        if (choice === "1") {
          ...
          else if (choice === "8") { await bot.runSpotTrade(); }
          else if (choice === "9") { await bot.runFuturesTrade(); }
          else if (choice === "10") {
